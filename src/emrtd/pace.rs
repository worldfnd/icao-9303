use {
    ...
    super::{Emrtd, secure_messaging::construct_secure_messaging},
    crate::{
        asn1::emrtd::security_info::PaceInfo,
...
use crate::crypto::{PublicKey, groups::{EllipticCurvePoint, EllipticCurve}, BsiTr031111Codec, Codec};
...
impl Emrtd {
    pub fn pace(&mut self, _rng: &mut impl CryptoCoreRng, mrz: &str, info: &PaceInfo) -> Result<()> {
        ...
        ensure!(rx_mac_pubk == mac_pubk,
            "Received authentication code doesn't match own");

        self.set_secure_messaging(construct_secure_messaging(info.protocol.cipher.unwrap(), &k, 0));
...
use {
    super::secure_messaging::{aes::*, tdes::*, Cipher},
    crate::asn1::emrtd::security_info::SymmetricCipher,
};
fn decrypt_nonce(nonce: &mut [u8], mrz: &str, info: &PaceInfo) {
    // Derive symmetric key K_pi
    let k = k_from_mrz(mrz);
    let k_pi = kdf_128(&k[..], KDF_PACE);
    match info.protocol.cipher.unwrap() {
        SymmetricCipher::Tdes => {
            let cipher = TDesCipher::from_key(&k_pi, &[0; 16]);
            cipher.dec_with_iv(nonce, &[0; 8]);
        }
        SymmetricCipher::Aes128 => {
            let cipher = Aes128Cipher::from_key(&k_pi, &[0; 16]);
            cipher.dec_with_iv(nonce, &[0; 16]);
        }
        SymmetricCipher::Aes192 => {
            let cipher = Aes192Cipher::from_key(&k_pi, &[0; 16]);
            cipher.dec_with_iv(nonce, &[0; 16]);
        }
        SymmetricCipher::Aes256 => {
            let cipher = Aes256Cipher::from_key(&k_pi, &[0; 16]);
            cipher.dec_with_iv(nonce, &[0; 16]);
        }
    };
}
...
fn compute_auth_token(shared_secret: &[u8], pubk: &[u8], info: &PaceInfo) -> Result<[u8; 8]> {
    use super::secure_messaging::{aes::*, Cipher};
    ...
    println!("kmac: {}", hex::encode(&cipher.kmac));

    Ok(cipher.mac(0, &encodedpubk))
}
...
fn ecpubkeyser_to_ecpubkey(bytes: &[u8], curve: &EllipticCurve<U521>) -> ECPublicKey<U521> {
    let point: EllipticCurvePoint<'_, U521> =
        BsiTr031111Codec::default()
        .decode(&mut &bytes[..], curve).unwrap();
    ECPublicKey {
        curve: curve.clone(),
...
mod tests {
    ...
    use {
        crate:: crypto::{BsiTr031111Codec, Codec, KeyAgreementAlgorithm, PublicKey},
        der::Decode,
        hex_literal::hex,
        rand::thread_rng,
    };
    ...
    fn test_pace_example_g1() {
        ...
        let pace_info = PaceInfo::from_der(&hex!(
        ...
        decrypt_nonce(&mut nonce, mrz, &pace_info);
        assert_eq!(nonce, hex!("3F00C4D3 9D153F2B 2A214A07 8D899B22"));

        // Generic mapping
        let map_i_privk = hex!("7F4EF07B 9EA82FD7 8AD689B3 8D0BC78C
                                F21F249D 953BC46F 4C6E1925 9C010F99");
        let map_i_pubk = hex!("04
        ...
        let map_c_privk = hex!("498FF497 56F2DC15 87840041 839A8598
        ...
        let public = ECPublicKey { 
            curve: curve.clone(),
            point: (U521::from_be_slice(&map_c_pubk.0), U521::from_be_slice(&map_c_pubk.1))
        };
        let (h_point, serialized) = ecka(private, &public).unwrap();
        assert_eq!(serialized, h.0);

        // Check G^
        let g_hat_computed = curve.generator() * curve.scalar_field().from(U521::from_be_slice(&nonce)) + h_point;
        ...
        let c_privk = hex!("107CF586 96EF6155 053340FD 633392BA
        ...
        let k = hex!("28768D20 701247DA E81804C9 E780EDE5
                                82A9996D B4A31502 0B273319 7DB84925");
        let kaa: Box<dyn KeyAgreementAlgorithm> = Box::new(curve.clone());
        // Check K (inspection)
        let private = curve.scalar_field().from(U521::from_be_slice(&i_privk)).as_montgomery();
        let public = ECPublicKey { 
            curve: curve.clone(),
            point: (U521::from_be_slice(&c_pubk.0), U521::from_be_slice(&c_pubk.1))
        };
        let serialized = kaa.key_agreement(&crate::crypto::PrivateKey(Box::new(private)), &PublicKey::EC(public)).unwrap();
        assert_eq!(serialized, k);

        // Check K (chip)
        let private = curve.scalar_field().from(U521::from_be_slice(&c_privk)).as_montgomery();
        let public = ECPublicKey { 
            curve: curve.clone(),
            point: (U521::from_be_slice(&i_pubk.0), U521::from_be_slice(&i_pubk.1))
        };
        let serialized = kaa.key_agreement(&crate::crypto::PrivateKey(Box::new(private)), &PublicKey::EC(public.clone())).unwrap();
        assert_eq!(serialized, k);

        let cipher = Aes128Cipher::from_seed(&k);
        assert_eq!(cipher.kenc, hex!("F5F0E35C 0D7161EE 6724EE51 3A0D9A7F"));
        assert_eq!(cipher.kmac, hex!("FE251C78 58B356B2 4514B3BD 5F4297D1"));
        ...
        let tifd = cipher.mac(0, &indata_tifd);
        ...
        let tic = cipher.mac(0, &indata_tic);
